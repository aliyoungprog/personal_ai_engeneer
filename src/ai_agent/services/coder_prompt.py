...
def build_coder_prompt(task: Task, body: str = "") -> str:
    ...
    parts += [
        ...
        "- A targeted inline `# noqa: <CODE>` is OK for a genuinely-spurious lint rule on",
        "  one line (e.g. `SLF001` when a test mocks a private member); never blanket-",
        "  disable rules or edit ruff config.",
...
def build_fix_prompt(task: Task, feedback: str, body: str = "") -> str:
    ...
    parts += [
        ...
        "- You MAY silence a single genuinely-spurious lint diagnostic with a targeted",
        "  inline `# noqa: <CODE>` on that exact line (common in tests, e.g. `SLF001`",
        "  when mocking a private member, or `S101` for asserts). Never blanket-disable",
        "  rules or edit ruff config to dodge real problems.",
        "- Add or update tests for any new logic.",
